...
impl ColumnVector for LiteralValueVector {
    ...
    fn get_value(&self, i: usize) -> Option<Box<dyn Any>> {
        ...
        self.value.as_ref().map(|v| match &self.arrow_type {
            DataType::Boolean => {
            ...
            DataType::Utf8 => {
                let val: &String = v
                    .downcast_ref()
                    .expect("Invalid value type, expected String");
...
mod tests {
    ...
    fn test_get_type() {
        let vector =
            LiteralValueVector::new(DataType::Int64, Some(Box::new(42i64) as Box<dyn Any>), 1);
    ...
    fn test_get_value_at_valid_index() {
        let value = 42i64;
        let vector =
            LiteralValueVector::new(DataType::Int64, Some(Box::new(value) as Box<dyn Any>), 1);
    ...
    fn test_len() {
        let vector =
            LiteralValueVector::new(DataType::Int64, Some(Box::new(42i64) as Box<dyn Any>), 3);
    ...
    fn test_clone() {
        ...
        assert_eq!(cloned_vector.get_type(), DataType::Int64);

        let result = cloned_vector.get_value(0);
        assert!(result.is_some());
